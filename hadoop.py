...
while True:
    ...
    elif (("stop" in ch) and (("slave" in ch) or ("datanode" in ch))):
    ...
    elif (("start" in ch) and (("slave" in ch) or ("datanode" in ch))):
        remote_ip=input("Enter the namenode ip which you want to stop :")
        os.system("ssh -l root {} hadoop-daemon.sh start datanode".format(remote_ip))
        os.system("clear")
          
    elif ((("check" in ch) or ("see" in ch)) and (("hadoop cluster report" in ch) or ("report" in ch))):
